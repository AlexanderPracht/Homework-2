...
bikes_average <- bikes1 %>%
                  filter(year %in% c(2015: 2019)) %>%
                  group_by(month) %>%
                  summarise(expected_rentals=mean(actual_rentals))


bikes1 <-left_join(bikes1,bikes_average, by="month")

bikes_monthly <- bikes1 %>%
  mutate(diff_month = actual_rentals-expected_rentals)

bikes_monthly$month <- as.numeric(bikes_monthly$month) # Months to numeric
...
labs(x=NULL, y="Bike Rentals", caption="Source: TfL, London Data Store", title="Monthly changes in TfL bike rentals", subtitle= "Change from monthly average shown in blue \nand calculated between 2015-2019") + theme_minimal(base_family="Arial") + theme (plot.title = element_text(size=10), plot.subtitle = element_text(size=9))+
  #+ geom_line(aes(x=month, y = expected_rentals), size=1.2, colour="blue") +
#+ geom_line(aes(x=month, y=actual_rentals), color="black", size=1.2)
  geom_ribbon(aes(ymin = expected_rentals + if_else(diff_month < 0, diff_month, 0),
                  ymax = expected_rentals), color ="black", fill = "red", alpha = 0.3) +
  geom_ribbon(aes(ymin = expected_rentals,
                  ymax = expected_rentals + if_else(diff_month > 0, diff_month, 0)),color ="black", fill = "green", alpha = 0.3)+ theme(aspect.ratio=0.5) + theme(axis.text.x= element_text(size=5)) +
