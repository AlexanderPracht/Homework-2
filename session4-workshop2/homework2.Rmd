...
I use left_join(), because left_join() enables to preserve the original observations even when there isn't a match. What's more, left_join() is faster than merge(). The worldbank_data has the most entries, so left join life_expectancy and then hiv data to it.
```{r}
hiv_tidy <- hiv %>% 
...
For most regions, including East Asia & Pacific, Latin America & Caribbean, Middle East & North Africa, there exists a negative relationship between HIV prevalence and life expectancy.
In Europe & Central Asia and Sub-Saharan Africa the relationship becomes slightly positive, which means a higher HIV prevalence with higher life expectancy. However this trend seems not to be significant.
In North America there are only two countries and therefore it is difficult to determine a trend.
South Asia shows also a negative trend, however also here it doesn't seem to be significant.
...
For all regions, the relationships between fertility rate and GDP per capita are negative: the higher the fertility rate, the lower the GDP per capita. However, there are some exceptional countries in each region.
```{r}
...
```
```{r, countries_mortality_improvements}
...
```
## Asia top 5 and flop 5 countries in mortality improvement
```{r, Asia}
...
## East Asia & Pacific top 5 and flop 5 countries in mortality improvement
```{r, east_asia}
...
```
## Europe & Central Asia top 5 and flop 5 countries in mortality improvement
```{r}
...
```
## Latin America & Caribbean top 5 and flop 5 countries in mortality improvement
```{r}
...
```
## Middle East & North Africa top 5 and flop 5 countries in mortality improvement
```{r}
...
```
## North America (only two countries) in mortality improvement
```{r}
...
north_america_top5
```
## Subsaharan Africa top 5 and flop 5 countries in mortality improvement
```{r}
...
```
We used the method clean_names() to have proper column descriptions separated by a "_".
...
```{r interest_data, echo=TRUE, fig.width=15}
...
```{r covid_death_rate_medcond, echo=TRUE, fig.width=15}
