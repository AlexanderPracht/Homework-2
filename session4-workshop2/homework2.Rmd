...
In this part, we will join a few dataframes on 
...
Firstly, we join the three dataframes: life_expectancy, worldbank_data, and HIV

We use left_join(), because left_join() enables to preserve the original observations even when there isn't a match. What's more, left_join() is faster than merge(). The worldbank_data has the most entries, so left join life_expectancy and then hiv data to it.
```{r}
hiv_tidy <- hiv %>% 
...
region_worldbank <- left_join(countries, worldbank_data, by = "country") %>%
...
We use full join within mutating joins because it helps us to keep all values in 3 dataframes and we don't want to lose any of the values. The main reason why we use full join is that we notice that some of the countries have different names in different dataframe. If we were to use left/right/inner joins, some data of the countries would be eliminated. 
Then, we generate a scatterplot with a regression line to investigate the relationship between HIV prevalence and life expectancy.
...
We observe that the influence of HIV Prevalence on Life Expectancy varies from region to region. In some regions, such as Europe & Central Asia and Latin America & Caribbean, the lower the HIV Prevalence, the higher the Life Expectancy, which shows that HIV Prevalence is a key factor of Life Expectancy. While in some other regions, such as South Asia and East Asia & Pacific, we can’t see a clear trend, which could be caused by the lack of data in those regions to show the trend.

Then, we generate a scatterplot with a regression line to investigate the relationship between fertility rate and GDP per capita.
...
We can see a clear negative relationship between GDP per capita and Fertility Rate in most regions, excluding Middle East & North Africa and Sub-Saharan .When GDP per capita increases, the fertility rate decreases. In the those regions being excluded, we observe a u-shape trend. When GDP per capita grows, the fertility rate firstly declines then goes up. There might be specific cultural factors or traditions that support them to give birth to more children when they have ability to raise up them.

We then generate a bar chart, in descending order to find out which regions have the most observations with missing HIV data.
...
We observe from the bar chart above that Sub-Saharan Africa have the most missing values in HIV data.

Then, we investigate how the mortality rate for under 5 changed by region.


Mortality rate for under 5 declined for all 7 regions over the time from 1960.
...
There is a negative relationship between primary school enrollment rate and fertility rate. The higher the fertility rate is, the lower the primary school enrollment rate. 
However, the relationship is not obvious in Europe & Central Asia, and North America, probably because the families in these two regions are wealthy enough to afford children's education, even with more children.
```{r}
...
In this section we will look at the [CDC Covid-19 Case Surveillance Data](https://data.cdc.gov/Case-Surveillance/COVID-19-Case-Surveillance-Public-Use-Data/vbim-akqf).
...
In this section we will look at the Tfl bike sharing data.
...
- Approximately how much time did you spend on this problem set: 6x8 hours
- What, if anything, gave you the most trouble: Styling the graphs
