...
email_data <- social_media%>%
  mutate(email = if_else(emailmin==NA, NA, emailhr*60 + emailmin))
```
...
Trump_approval_margin %>% 
  filter(enddate_week %in% c(15, 20), enddate_year == 2020) %>% 
  select(c(enddate_year, enddate_week, low_net_approval, high_net_approval))

```
Trump's net approval rate decreased greatly from 2020 week 15 to week 20, with larger interval. I think the reason of lower net approval rate is that most people were disappointed with Trump's inability to contain coronavirus. What's more, the greater deviation in net qpproval rate caused larger interval.
