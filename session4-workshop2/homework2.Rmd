...
<<<<<<< HEAD
  ...
  group_by(sex, age_group, medcond_yn) %>% #group by variables we need
  summarise (yes = sum(death_yn == "Yes"), no = sum(death_yn == "No") %>%
               mutate(death_rate = (yes/sum(yes+no))*100))
             
             

  ggplot(graph1_deaths, aes(x = age_group,y = death_rate)) +
  geom_col()  
...
```{r tfl_calc, echo=FALSE, out.width="100%", fig.width=20, fig.height=10}
bike_data <- bike%>%
  filter(year %in% c(2015:2019))%>%
  group_by(year,month)%>%
  summarise(bikes_month=sum(bikes_hired), n=n())%>%
  mutate(average_month_perday = bikes_month/n)%>%
  ungroup()%>%
  group_by(month)%>%
  summarise(avg_15_19=sum(average_month_perday)/n())%>%
  ungroup()

bike_data2 <- bike%>%
  filter(year %in% c(2015:2020))%>%
  group_by(year,month)%>%
  summarize(bikes_per_month = sum(bikes_hired)/n())

bike_data3 <- left_join(bike_data, bike_data2)

ggplot(bike_data3)+
  geom_line(aes(month, bikes_per_month, group=1))+
  geom_line(aes(month, avg_15_19, group=1), color = "blue")+
   facet_wrap(~year)+
  geom_ribbon(aes(month, ymin = bikes_per_month, ymax= pmin(bikes_per_month,avg_15_19),group = year,alpha=0.01),show.legend = FALSE,fill="green",size=0.0001)+
  geom_ribbon(aes(month, ymin = avg_15_19, ymax= pmin(bikes_per_month,avg_15_19),group = year,alpha=0.01),show.legend = FALSE,fill="red",size=0.0001)
  
 

# ggplot(bike_data3, aes(x=month, y = avg_15_19))+
#   geom_point()+
#     theme_bw() +
#   labs (
